...
impl Command for Lsr {
    ...
    fn run(
        ...
        let lsrargs = LsrArgs::new();
        let v = Walker::new(&lsrargs).walk_and_print();
        // eprintln!("v: {:?}", v);
        // Ok(v.into_iter().into_pipeline_data(engine_state.ctrlc.clone()))
        // Ok(v.into_pipeline_data(engine_state.ctrlc.clone()))

        v.map(|x| {
            x.into_pipeline_data_with_metadata(
...
pub struct LsrArgs<'a> {
    ...
    // added
    pub reg_exp_str: &'a str,
    pub exclude_reg_exp_str: Option<&'a str>,
    // pub contents_first: bool,
    // pub follow_links: bool,
    // // max_open // number of simultaneous open file descriptors used by the iterator
    // pub min_depth: Option<usize>,
    // pub same_file_system: bool,
    // added from ignore crate
    //  pub max_filesize: Option<u64>,
    //  pub standard_filters: bool, // hidden, parents, ignore, git_ignore, git_global, git_exclude
    //  pub hidden: bool,
    //  pub parents: bool,
    //  pub ignore: bool,
    //  pub git_global: bool,
...
impl LsrArgs<'_> {
    pub fn new() -> LsrArgs<'static> {
        LsrArgs {
            reg_exp: Regex::new(".*").unwrap(),
            reg_exp_str: ".*",
            ...
            exclude_reg_exp: None,
            exclude_reg_exp_str: None,
    ...
    fn search_pattern(&self) -> Regex {
        self.parse_regex_from_pattern_of(
            Some(&self.reg_exp_str),
            "Failed to parse the provided PATTERN:",
        )
    }

    fn root_path(&self) -> String {
        let root_path = self.root_path.as_str();
    ...
    fn exclude_reg_exp(&self) -> Option<Regex> {
        if self.exclude_reg_exp.is_some() {
            let reg_exp = self.parse_regex_from_pattern_of(
                self.exclude_reg_exp_str,
    ...
    fn parse_regex_from_pattern_of(&self, arg_name: Option<&str>, error_message: &str) -> Regex {
        // let input_pattern = self.matches.value_of(arg_name).unwrap();
        let input_pattern = match arg_name {
            Some(arg) => arg,
            None => "",
        };
    ...
    fn to_args(&self) -> LsrArgs {
        LsrArgs {
            ...
            // added
            reg_exp_str: self.reg_exp_str,
            exclude_reg_exp_str: self.exclude_reg_exp_str,
...
impl<'a> Walker<'a> {
    pub fn new(args: &'a LsrArgs) -> Walker<'a> {
        Walker { args }
    }

    pub fn walk_and_print(&self) -> Result<Value, ShellError> {
        use std::sync::mpsc;
        use std::thread;
        ...
        let walker = WalkBuilder::new(&self.args.root_path)
            ...
            // .sort_by_file_name(|a, b| a.file_name().cmp(b.file_name()))
        ...
        let print_thread = thread::spawn(move || -> io::Result<Value> {
            let mut entries = vec![];
            for path in rx.iter() {
                ...
                let metadata = match std::fs::symlink_metadata(&pb) {
                    Ok(metadata) => Some(metadata),
                    Err(_) => None,
                };
                let display_name = pb.display().to_string().replace(".\\", "");
                let entry = match dir_entry_dict(
                    &pb,
                    &display_name,
                    metadata.as_ref(),
                    Span::unknown(),
                    ...
                    Ok(entry) => entry,
                    Err(_) => Value::nothing(Span::unknown()),
                };
                // println!("{:?}", entry);
                eprintln!("added: {:?}", &path);
                entries.push(entry);
        ...
        let working_dir_path = binding.to_string_lossy();
