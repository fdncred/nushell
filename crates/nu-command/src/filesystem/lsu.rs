...
//     //     })
//     // }
//     pub fn get_default() -> UResult<Self> {
//         Ok(Self {
//             // Dir and vdir needs access to this field
//             format: Format::Long,
//             files: Files::All,
...
//             recursive: false,
//             reverse: false,
//             dereference: Dereference::None,
//             ignore_patterns: vec![], //Vec<Pattern>,
//             size_format: SizeFormat::Bytes,
//             directory: false,
//             time: Time::Modification,
//             #[cfg(unix)]
//             inode: false,
//             color: None, //Option<LsColors>,
//             long: LongFormat {
//                 author: true,
//                 group: true,
//                 owner: true,
...
//             if ((10.0 * bytes) as f32).ceil() >= 100.0 {
//                 format!("{:.0}{}", bytes.ceil(), prefix_str)
//             } else {
//                 format!("{:.1}{}", ((10.0 * bytes) as f32).ceil() / 10.0, prefix_str)
//             }
//         }
//     }
// }
...
impl Command for Lsu {
    fn name(&self) -> &str {
        "lsu"
    }

    fn signature(&self) -> Signature {
        Signature::build("lsu").category(Category::FileSystem)
    }

    fn usage(&self) -> &str {
        "core utils ls command"
    }
    ...
    fn run(
        ...
        // use uu_ls::Options;
        // let config = Config::get_default().unwrap();
        // let mut locs = vec![];
        // locs.push(Path::new("."));
        // // paths.push(Path::new("."));
        // // let locs = paths
        ...
        use std::path::Path;
        use uu_ls::{list, options, uu_app, uumain, Config};
        use uucore::quoting_style::{Quotes, QuotingStyle};
        use uucore::Args;

        // use uucore::Args::args;
        // uucore::bin!(uu_ls);
        let cmd = uu_ls::uu_app();
        let matches = cmd.get_matches_from(vec!["-l", "-a"]);
        // let matches = cmd.get_matches_from(args);
        let cfg = Config::from(&matches).unwrap();
        // cfg.quoting_style = QuotingStyle::C {
        //     quotes: Quotes::None,
        // };
        // cfg.format = uu_ls::Format::Long;
        // uu_ls::uumain(uucore::args_os());
        if let Ok(config) = Config::from(&matches) {
            let locs = matches
                .get_many::<OsString>(options::PATHS)
                .map(|v| v.map(Path::new).collect())
                .unwrap_or_else(|| vec![Path::new(".")]);
            // uu_ls::uumain(locs.into_iter());
            list(locs, &config);
        }
